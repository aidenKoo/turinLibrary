...
<%
	...
	try {
		...
		// 게시물의 총 수를 얻는 쿼리 실행
		pstmt = conn.prepareStatement("SELECT COUNT(num) AS TOTAL FROM tblqnaboard" + whereSQL);
		if (!"".equals(whereSQL)) {
		...
		// 게시물 목록을 얻는 쿼리 실행
		pstmt = conn.prepareStatement("SELECT * FROM tblqnaboard "+whereSQL+" ORDER BY num DESC LIMIT ?, ?");
		if (!"".equals(whereSQL)) {
			// 전체검색일시
			if ("ALL".equals(searchType)) {
				pstmt.setString(1, searchTextUTF8);
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Q&A List</h1>
		...
		<div class="row">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<table data-toggle="table"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name"  data-sort-order="desc">
						    ...
						    <thead>
						    <tr>
						        <th data-checkbox="true" >num</th>
						        <th  data-sortable="true">num</th>
						        <th  data-sortable="true">title</th>
						        <th  data-sortable="true">memberID</th>
						        <th   data-sortable="true">writedate</th>
						        <th data-sortable="true">owner</th>
						        <th data-sortable="true">hasAnswer</th>
						    </tr>
						    </thead>
						    <tbody>
			<%
			...
			<tr>
				...
				<td><a href="6_admin_qnaboard_output.jsp?num=<%=rs.getString("num")%>&amp;pageNum=<%=pageNumTemp%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>"><%=rs.getString("title") %></a></td>
				<td align="center"><%=rs.getString("memberID") %></td>
				<td align="center"><%=rs.getString("writedate") %></td>
				<td align="center"><%=rs.getString("owner") %></td>
				<td align="center"><%=rs.getString("hasAnswer") %></td>
			</tr>
			<%
				}
			}
